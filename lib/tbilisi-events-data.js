...
var VALID_SOURCE_TYPES =['telegram', 'website', 'facebook', 'facebook_group', 'instagram', 'tkt', 'biletebi'];
...
var EVENT_DEFAULTS = {
  ...
  viewCount: 0,
  // titleKey: normalized title, used to spot other occurrences of the same
  // recurring event. seriesId: shared by every occurrence once at least two
  // are linked (the id of the earliest-dated one) — see resolveSeriesId() in
  // tbilisi-events-pipeline.js.
  titleKey: null, seriesId: null,
};
...
async function getEventsByTitleKey(titleKey) {
  if (!titleKey) return [];
  var snap = await eventsCollection().where('titleKey', '==', titleKey).get();
  return snap.docs.map(function(d) { return Object.assign({ id: d.id }, d.data()); });
}

async function getEventsBySeriesId(seriesId) {
  if (!seriesId) return [];
  var snap = await eventsCollection().where('seriesId', '==', seriesId).get();
  return snap.docs.map(function(d) { return Object.assign({ id: d.id }, d.data()); });
}
...
function eventMatchesFilters(e, f) {
  ...
  if (f.seriesId && e.seriesId !== f.seriesId) return false;
...
var VENUE_DEFAULTS = {
  ...
  type: null, description: null, imageUrl: null, images: [], website: null,
  hours: null, social: null,
  eventCount: 0, origin: 'auto', researchedAt: null, researchNote: null,
  editorVerified: false, editorsPick: false,
...
// Gallery: `images` is the full set of uploaded photos, `imageUrl` is the cover
// shown in cards/hero. Appending/removing goes through a transaction so
// concurrent uploads (a multi-file form submit) don't clobber each other.
async function addVenueImage(id, url) {
  await _db.runTransaction(async function(t) {
    var ref = venuesCollection().doc(id);
    var snap = await t.get(ref);
    if (!snap.exists) return;
    var d = snap.data();
    var images = (d.images || []).slice();
    if (images.indexOf(url) === -1) images.push(url);
    var patch = { images: images, updatedAt: new Date() };
    if (!d.imageUrl) patch.imageUrl = url; // first photo becomes the cover by default
    t.update(ref, patch);
  });
}

async function removeVenueImage(id, url) {
  await _db.runTransaction(async function(t) {
    var ref = venuesCollection().doc(id);
    var snap = await t.get(ref);
    if (!snap.exists) return;
    var d = snap.data();
    var images = (d.images || []).filter(function(u) { return u !== url; });
    var patch = { images: images, updatedAt: new Date() };
    if (d.imageUrl === url) patch.imageUrl = images[0] || null;
    t.update(ref, patch);
  });
}

async function setVenueCoverImage(id, url) {
  await updateVenue(id, { imageUrl: url });
}
...
module.exports = {
  ...
  getEventsByTitleKey: getEventsByTitleKey,
  getEventsBySeriesId: getEventsBySeriesId,
  getEvents: getEvents,
  ...
  addVenueImage: addVenueImage,
  removeVenueImage: removeVenueImage,
  setVenueCoverImage: setVenueCoverImage,
