...
async function researchVenue(name) {
  ...
  var districtSlugsByCity = {};
  Object.keys(taxonomy.DISTRICTS || {}).forEach(function(c) {
    districtSlugsByCity[c] = (taxonomy.DISTRICTS[c] || []).map(function(d) { return d.slug; });
  });
  ...
  var prompt = 'Venue name as it appears in local Tbilisi event listings: "' + name + '".\n\n'
    ...
    + '  "address": string | null,\n'
    + '  "area": string | null,\n'
    + '  "city": one of ' + JSON.stringify(taxonomy.CITY_SLUGS) + ' | null,\n'
    + '  "district": the matching district slug for that city, or null. Districts by city: '
    + JSON.stringify(districtSlugsByCity) + '\n'
    + '  "lat": number | null,\n'
    + '  "lng": number | null,\n'
    + '  "website": string | null,\n'
    ...
    + '}\n'
    + 'area: the free-text neighbourhood name (fill even if it is not in the district list). '
    + 'district: only set it if the venue is clearly in one of the listed districts.\n'
  ...
  var city = (taxonomy.CITY_SLUGS || []).indexOf(j.city) !== -1 ? j.city : null;
  var district = null;
  if (city && j.district) {
    var ds = (taxonomy.DISTRICTS[city] || []).map(function(d) { return d.slug; });
    if (ds.indexOf(j.district) !== -1) district = j.district;
  }
