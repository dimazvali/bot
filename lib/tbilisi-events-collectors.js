...
// The Cloudflare block on tkt.ge/biletebi.ge appears intermittent (has
// succeeded from the same production host on other runs), so a short retry
// with backoff is worth it before giving up on the source.
async function getWithRetry(url, options, attempts) {
  attempts = attempts || 3;
  var lastErr;
  for (var i = 0; i < attempts; i++) {
    try {
      ...
      lastErr = err;
      if (i < attempts - 1) {
        await new Promise(function(resolve) { setTimeout(resolve, 4000 * (i + 1)); });
      }
    }
  }
  throw lastErr;
...
async function collectTkt() {
  var res = await getWithRetry(TKT_GATEWAY + '/shows/list', { timeout: 20000, headers: BROWSER_HEADERS });
...
async function collectBiletebi() {
  ...
  do {
    ...
    var res = await getWithRetry(url, { timeout: 20000, headers: Object.assign({ 'Referer': 'https://biletebi.ge/' }, BROWSER_HEADERS) });
    var body = res.data || {};
