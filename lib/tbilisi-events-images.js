...
// Store an already-in-memory upload (admin event photo) as webp. Returns the URL.
async function storeEventImage(buffer, eventId) {
...
// `suffix` disambiguates several files uploaded in the same request (same
// Date.now() millisecond) — e.g. a multi-file gallery upload.
async function storeVenueImage(buffer, venueId, suffix) {
  var webp = await toWebp(buffer);
  var path = 'tbilisi-events/venues/' + venueId + '/' + Date.now() + (suffix != null ? '-' + suffix : '') + '.webp';
  return saveBuffer(webp, path);
}
...
module.exports = { init: init, fetchAndStore: fetchAndStore, storeEventImage: storeEventImage, storeVenueImage: storeVenueImage, storeHeroImage: storeHeroImage };
