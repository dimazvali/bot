        ...
        if (payload.startsWith('reg_')) {
          ...
          if (!regSnap.exists) {
            ...
            if (regData.tg_user_id) {
              ...
              let newsData = null;
              if (regData.news_id) {
                const newsSnap = await _db.collection('pelamushi_news').doc(regData.news_id).get();
                if (newsSnap.exists) newsData = newsSnap.data();
              }
              const eventTitle = newsData ? (newsData.title_ru || newsData.title_en || '') : '';

              let dateLine = '';
              if (newsData && newsData.event_date) {
                const d = newsData.event_date.toDate ? newsData.event_date.toDate() : new Date(newsData.event_date);
                dateLine = d.toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })
                  + ', ' + d.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' });
              }

              let text = 'Спасибо! 🎉\n\nЯ привязан к вашей регистрации. Здесь вы будете получать уведомления по вашему событию.';
              if (dateLine) text += '\n\n📅 ' + dateLine;
              text += `\n\nЖдём вас на событии${eventTitle ? ' «' + eventTitle + '»' : ''}!\nЕсли что-то пойдёт не так — или возникнут вопросы, просто напишите боту.`;

              const photoUrl = newsData && newsData.photo_url;
              if (photoUrl) {
                await sendMedia(msg.chat.id, 'photo', photoUrl, text).catch(() => sendMessage(msg.chat.id, text).catch(() => {}));
              } else {
                await sendMessage(msg.chat.id, text).catch(() => {});
              }
            }
          }
        } else {
