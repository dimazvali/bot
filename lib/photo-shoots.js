...
async function initFromFirestore(db) {
  ...
  shootSnap.docs.forEach(function(doc) {
    ...
    result[slug] = {
      ...
      showCuratorSelection: !!d.showCuratorSelection,
      relatedShoots: d.relatedShoots || [],
      mapCenter: d.mapCenter || null,
...
async function updatePhoto(slug, photoId, fields) {
  var env = getEnv();
  var safe = {
    title: fields.title || '', title_en: fields.title_en || '',
    date: fields.date || '',
    desc: fields.desc || '', desc_en: fields.desc_en || '',
    type: ['copter','camera','mobile'].includes(fields.type) ? fields.type : 'camera',
    panorama: !!fields.panorama,
    curatorPick: !!fields.curatorPick,
    coords: fields.coords || null,
    colorFamilies: (fields.colorFamilies && fields.colorFamilies.length) ? fields.colorFamilies : [],
  };
...
async function updatePhotoSeo(slug, photoId, seoDesc, seoKeywords, seoDescEn, seoKeywordsEn) {
  var env = getEnv();
  var safe = { seo_desc: seoDesc || '', seo_keywords: seoKeywords || '', seo_desc_en: seoDescEn || '', seo_keywords_en: seoKeywordsEn || '' };
  await _db.collection('shootPhotos').doc(env + '_' + slug + '_' + photoId).update(safe);
  if (_cache && _cache[slug]) {
    var photo = _cache[slug].photos.find(function(p) { return p.id === photoId; });
    if (photo) Object.assign(photo, safe);
...
// Used when replacing a photo's source file: overwrites the derived-asset fields
// (urls/dimensions/color/faces) in place, leaving title/desc/SEO/annotations untouched.
async function updatePhotoAssets(slug, photoId, fields) {
  var env = getEnv();
  await _db.collection('shootPhotos').doc(env + '_' + slug + '_' + photoId).update(fields);
  if (_cache && _cache[slug]) {
    var photo = _cache[slug].photos.find(function(p) { return p.id === photoId; });
...
async function updatePhotoCuratorPick(slug, photoId, value) {
  var env = getEnv();
  await _db.collection('shootPhotos').doc(env + '_' + slug + '_' + photoId).update({ curatorPick: !!value });
  if (_cache && _cache[slug]) {
    var photo = _cache[slug].photos.find(function(p) { return p.id === photoId; });
    if (photo) photo.curatorPick = !!value;
  }
}
...
module.exports = { initFromFirestore, getData, getShoot, createShoot, saveShoot, setRelatedShoots, addPhoto, updatePhoto, updatePhotoSeo, updatePhotoFaces, updatePhotoAssets, updatePhotoCuratorPick, removePhoto, reorderPhotos, addAnnotation, moveAnnotation, removeAnnotation, deleteShoot, addCollection, getCollections };
