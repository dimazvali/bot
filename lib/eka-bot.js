...
async function handleUpdate(update, onNewMessage, onCommand, onNewUser, onStartPayload) {
  try {
    ...
    if (msg && msg.from && !msg.from.is_bot) {
      ...
      if (isStart) {
        ...
        } else if (typeof onStartPayload === 'function' && payload && await onStartPayload(payload, msg)) {
          // handled by the caller (e.g. tbilisi-events te_ deep link)
        } else {
...
module.exports = { init, setWebhook, getBotProfile, setBotProfile, registerUser, markInactive, markActive, sendMessage, sendMedia, sendLocation, forwardMessage, saveMessage, getUsers, getBotMessages, saveBotMessages, handleUpdate };
