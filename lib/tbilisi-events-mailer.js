...
// bodyHtml is inserted VERBATIM — no sanitization here. Every caller
// (lib/tbilisi-events-notify.js) MUST HTML-escape any interpolated value.
function buildNotificationEmail(subject, bodyHtml, lang) {
  var html = '<!DOCTYPE html><html><body style="margin:0;background:#faf6f0;">'
    + '<div style="font-family:Arial,Helvetica,sans-serif;max-width:480px;margin:0 auto;padding:32px 24px;color:#14110e;">'
    + '<p style="font-size:11px;letter-spacing:2px;text-transform:uppercase;color:#8b8175;margin:0 0 18px;">events.tbiliseli.com</p>'
    + '<h1 style="font-size:18px;font-weight:700;margin:0 0 14px;">' + escHtml(subject) + '</h1>'
    + '<div style="font-size:14px;line-height:1.55;">' + bodyHtml + '</div>'
    + '</div></body></html>';
  var text = String(subject) + '\n\n'
    + String(bodyHtml).replace(/<[^>]+>/g, '').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&#39;/g, "'").replace(/&quot;/g, '"').trim()
    + '\n\nevents.tbiliseli.com';
  return { subject: subject, html: html, text: text };
}

async function sendUserNotification(email, msg) {
  if (!transporter) return;
  await transporter.sendMail({
    from: '"events.tbiliseli.com" <' + process.env.GMAIL_USER + '>',
    to: email, subject: msg.subject, html: msg.html, text: msg.text,
  });
}

module.exports = { init: init, buildMagicLinkEmail: buildMagicLinkEmail, sendMagicLink: sendMagicLink, copyFor: copyFor, buildNotificationEmail: buildNotificationEmail, sendUserNotification: sendUserNotification };
