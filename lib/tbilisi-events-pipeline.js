...
var STRUCTURED_COLLECTOR_BY_TYPE = {
  ...
  biletebi: function() { return collectors.collectBiletebi(); },
};
...
// The same real-world event (a weekly night, a touring show with several
// dates) is still stored as one document per occurrence — each date can have
// its own sources/price/cancellation — but occurrences that share a title
// (and, when both are known, a venue) get linked with a common seriesId so
// the front end can show one card with "+ N dates" instead of N duplicates.
// Deliberately title-exact + no LLM call: unlike the same-day dedup match,
// there is no second signal (same date) to sanity-check a fuzzy/translated
// title against, so a stricter, cheaper rule is safer here. Admins can always
// see every occurrence individually in /admin/events.
async function resolveSeriesId(extracted) {
  var titleKey = dedup.normalizeTitle(extracted.title);
  if (!titleKey) return null;
  var today = new Date().toISOString().slice(0, 10);
  var placeKey = venues.normalizeVenueName(extracted.place || '');

  var sameTitle = await data.getEventsByTitleKey(titleKey);
  var candidates = sameTitle.filter(function(e) {
    if (e.date === extracted.date) return false; // same-day dupes go through the dedup match above, not here
    if (!e.date || e.date < today) return false; // don't chain onto occurrences already in the past
    if (placeKey && e.place) return venues.normalizeVenueName(e.place) === placeKey;
    return true; // no place on one side to compare — title match alone still stands
  });
  if (!candidates.length) return null;

  var anchor = candidates.find(function(e) { return e.seriesId; });
  if (anchor) return anchor.seriesId;

  // First time a second occurrence shows up: mint the group id from the
  // earliest-dated candidate and retro-tag it so both docs end up linked.
  candidates.sort(function(a, b) { return a.date.localeCompare(b.date); });
  var first = candidates[0];
  await data.updateEvent(first.id, { seriesId: first.id });
  return first.id;
}
...
async function persistEvent(extracted, sourceLabel, sourceUrl, summary, rawText, imageCandidate, onUsage) {
  ...
  try {
    ...
    var seriesId = null;
    try {
      seriesId = await resolveSeriesId(extracted);
    } catch (e) {
      summary.sourceErrors.push({ source: sourceLabel, error: 'series link failed: ' + e.message });
    }

    var rawExcerpt = rawText ? String(rawText).slice(0, 1500) : null;
    var id = await data.insertEvent(Object.assign({}, extracted, {
      dedupeKey: dedupeKey,
      titleKey: dedup.normalizeTitle(extracted.title),
      seriesId: seriesId,
      sources: [sourceEntry],
    ...
    if (structuredCollect) {
      ...
      for (var m = 0; m < structuredEvents.length; m++) {
        ...
        var evId = await persistEvent(
          { title: ev.title, date: ev.date, time: ev.time, place: ev.place, type: ev.type || null, language: [], price: ev.price || null },
...
module.exports = { run: run, resolveSeriesId: resolveSeriesId };
