...
async function sendReviewRequest(tour) {
  var siteUrl = process.env.EKA_SITE_URL || 'https://tbiliseli.com';
  var requests = await ekaData.getRequests({ tourId: tour.id });
  var seen = {};
  var targets = requests.filter(function(r) {
    if (!r.tg_user_id || r.status === 'declined' || r.status === 'cancelled') return false;
  ...
  for (var i = 0; i < targets.length; i++) {
    var r = targets[i];
    var lang = r.lang || 'ru';
    var ticketUrl = siteUrl + '/' + lang + '/ticket/' + r.id;
    var text = lang === 'en'
      ? '🙏 Hope you enjoyed the tour <b>' + (tour.titleEn || nameRu) + '</b>!\n\nWe\'d love to hear your thoughts — it takes just a minute:\n' + ticketUrl
      : '🙏 Надеемся, тур <b>' + nameRu + '</b> вам понравился!\n\nБудем очень рады вашему отзыву — это займёт минуту:\n' + ticketUrl;
    try {
      await ekaBot.sendMessage(r.tg_user_id, text);
    } catch(e) {
      console.error('[eka-reminders] review send to', r.tg_user_id, e.message);
    }
    if (i < targets.length - 1) await new Promise(function(r2) { setTimeout(r2, 50); });
  }
