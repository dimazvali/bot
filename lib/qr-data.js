...
// Optional map coordinates — null (not "not on the map yet") rather than a
// throw, since most entries won't have these set and that's fine.
...
function createStore(collection, opts) {
  ...
  async function create(entry) {
    ...
    var record = {
      ...
      lat: normalizeCoord(entry.lat),
      lng: normalizeCoord(entry.lng),
      views: 0,
      createdAt: now,
      updatedAt: now,
    };
  ...
  async function update(slug, patch) {
    ...
    if ('lat' in normalizedPatch) normalizedPatch.lat = normalizeCoord(normalizedPatch.lat);
    if ('lng' in normalizedPatch) normalizedPatch.lng = normalizeCoord(normalizedPatch.lng);
...
// Separate collection for local/dev (process.env.develop=true) so testing
// never reads or writes real qr.dimazvali.com entries.
var COLLECTION_NAME = qrFirebase.isDev ? 'qrPhotos_dev' : 'qrPhotos';
var defaultStore = createStore(qrFirebase.db ? qrFirebase.db.collection(COLLECTION_NAME) : unavailableCollection());
