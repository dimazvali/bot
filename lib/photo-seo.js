...
var BASE_KEYWORDS = 'фотография, photography, Дмитрий Шестаков, Dmitry Shestakov, документальная фотография, documentary photography';
...
async function generatePhotoSeo(photo, { countryLabel, seriesLabel, allTags, shootDesc, previousCaptions, knownPeople }) {
  ...
  var typeLabel = { copter: 'aerial drone photography', camera: 'ground-level camera photography', mobile: 'mobile phone photography' }[photo.type] || 'photography';
  ...
  content.push({
    ...
    text: 'Посмотри на фотографию и опиши, что на ней изображено — конкретную сцену, деталь, предмет.\n\n'
      ...
      + 'Затем сделай то же самое на английском: короткую подпись в том же стиле (сухо, точно, безоценочно, максимум 110 символов) и отдельный набор SEO keywords на английском — это не дословный перевод русской подписи, а тот же принцип съёмки заново на английском.\n\n'
      ...
      + 'Ответь только JSON, без markdown: {"desc":"...","keywords":"...","desc_en":"...","keywords_en":"..."}',
  });
  ...
  return {
    ...
    descEn: truncateDesc(json.desc_en, 150),
    keywordsEn: (json.keywords_en || '').trim(),
  };
}
...
// Backfills missing EN fields by translating the RU text the admin just typed
// (not a fresh AI-written caption like generatePhotoSeo — a literal translation
// of what's there). Only asks for whichever of title/desc is actually missing;
// returns {} untouched if nothing needs translating, so callers can skip the
// API round-trip entirely on the common case (both already filled).
async function translateTitleDesc({ title, desc }) {
  var parts = [];
  ...
  var apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error('ANTHROPIC_API_KEY not set');
  var client = new Anthropic({ apiKey, timeout: 20000 });
  ...
  var message = await client.messages.create({
    ...
    messages: [{
      role: 'user',
      content: 'Переведи на английский следующие поля подписи к фотографии — естественно, в том же лаконичном стиле, не дословно. '
        + 'Ответь только JSON без markdown, с теми же ключами, что даны ниже:\n\n' + parts.join('\n'),
    }],
  });
  ...
  var json = JSON.parse(match[0]);
  var result = {};
  if (title && json.title) result.title = String(json.title).trim();
  if (desc && json.desc) result.desc = String(json.desc).trim();
  return result;
}
...
module.exports = { generatePhotoSeo, translateTitleDesc, buildPageKeywords, BASE_KEYWORDS };
