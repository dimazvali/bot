...
async function sendRequestNotification(request) {
  ...
  var subject = request.type === 'question'
    ? 'Вопрос: ' + (request.tourTitle || request.directionSlug || '')
    : request.type === 'tour'
    ? 'Новая заявка на тур: ' + (request.tourTitle || '')
    : 'Новая заявка на направление: ' + (request.directionSlug || '');
  var contactLine = (request.contactType || 'email').toUpperCase() + ': ' + (request.contact || '');
...
async function sendQuestionNotification(request, questionText, tourTitle) {
  if (!transporter || !process.env.EKA_NOTIFY_EMAIL) return;
  var subject = 'Вопрос от ' + (request.name || 'гостя') + (tourTitle ? ' — ' + tourTitle : '');
  var html = '<!DOCTYPE html><html><body style="margin:0;padding:0;background:#0d0d0d;">'
    + '<div style="font-family:monospace;max-width:520px;margin:0 auto;background:#0d0d0d;color:#ccc;padding:32px 24px;">'
    + '<p style="font-size:9px;letter-spacing:3px;color:#555;margin:0 0 20px;">EKA.DIMAZVALI.COM · ВОПРОС ОТ ГОСТЯ</p>'
    + '<h2 style="font-size:16px;font-weight:400;color:#fff;margin:0 0 20px;">' + escHtml(subject) + '</h2>'
    + '<table style="width:100%;border-collapse:collapse;">'
    + makeRow('Имя', request.name)
    + makeRow('Контакт', (request.contactType || 'email').toUpperCase() + ': ' + (request.contact || ''))
    + makeRow('Вопрос', questionText)
    + '</table>'
    + '</div></body></html>';
  await transporter.sendMail({
    from: '"eka.dimazvali.com" <' + process.env.GMAIL_USER + '>',
...
module.exports = { init, sendRequestNotification, sendQuestionNotification };
