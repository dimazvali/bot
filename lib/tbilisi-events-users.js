...
// redirected to the login page with a safe ?next=; XHR/JSON requests get 401.
function requireUser(req, res, next) {
  if (res.locals.user) return next();
  if (wantsJson(req)) return res.status(401).json({ error: 'auth_required' });
  var back = safeNext(req.originalUrl);
  return res.redirect('/tbilisi-events/login?next=' + encodeURIComponent(back));
}
