  ...
  if (unsubBtn) {
    unsubBtn.addEventListener('click', function () {
      ...
      fetch('/unsubscribe/google', { method: 'POST' })
        ...
        .then(function (d) { if (d.ok) { window.photoTrack && window.photoTrack('unsubscribe', { method: 'email' }); window.location.reload(); } else unsubBtn.disabled = false; })
      ...
      google.accounts.id.initialize({
        ...
        callback: function (response) {
          fetch('/subscribe/google', {
            ...
            .then(function (r) { return r.json(); })
            .then(function (data) {
              if (data.ok) { window.photoTrack && window.photoTrack('subscribe', { method: 'email' }); window.location.reload(); }
